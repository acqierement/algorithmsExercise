...
### 堆排序
极客时间：数据结构与算法之美：[28 | 堆和堆排序](https://time.geekbang.org/column/article/69913)
堆是一个完全二叉树，堆有这样的性质，其实是完全二叉树有这样的性质，对于每个节点他的位置是i，那么他的左子树就是i * 2，右子树就是i * 2+1.假设父子树是第一个数据 i = 1，那么左子树就是 i*2 = 2 的位置。这里根节点是从1开始的，但是数组是从0开始的，所以过程中需要进行转换，不注意比较容易出错。
第一步先建堆，从第一个有子树的节点开始进行堆化，堆化就是保证当前节点就是子树中的最大值，我们从底部一个个往上堆化，最终可以保证堆是一个最大堆。然后进行排序，把第一个数据（此时已经是最大值）与最后一个数据交换，这样数组的最后一个数据就是最大值了，而第一个数据此时就是我们刚交换过来的，我们只需对这个数据进行堆化，注意此时堆化的数组要排除掉最后一个数据。
